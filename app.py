...
def view(file_id):
    ...
    headers = {                             # build Box's authorization header
    ...
    for i in range(30):      # for loop to give the files 30 secs to process
        ...
        api_response = requests.get(url, headers=headers)                     # GET request for the doc

        status = api_response.json()['status']    # waiting for the document processing to finish
        if status == 'done':
            break
        elif status == 'error':
            break
        else:
            time.sleep(1)       # waits up to 30 secs then failed
    ...
    view_url = '{}/{}/view?theme=dark'.format(s.SESSIONS_URL, session_id)    # obtains the URL with session id w/ theme
...
def get_folder_items(folder_id=0):
    ...
    api_response = requests.get(url, headers=auth)    # make request
    api_response.raise_for_status()                   # check for status

    return api_response.json()          # JSON full of items details
...
def get_boxcloud_for_file(file_id):
    ...
    api_response = requests.get(url, headers=auth, allow_redirects=False) # makes request
    ...
    return boxcloud_link      # returns the URL to the file
